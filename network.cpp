...
void Network::update() {
	for(size_t i=0; i < n_neurons; ++i) {
		...
		if(neurons_[i].update(Inull, J_e*pois_dis_(generator))) {
...
void Network::update(double Iext) {
	for(size_t i=0; i < neurons_.size(); i++) {
		if (i == 0) {
			neurons_[i].update(Iext, 0);
		}
		else {
			neurons_[i].update(Inull, 0);
		}
	}	
	if(neurons_[0].getSpike()) {
		/** Here, I don't use the boolean return of update because I don't want the neuron to update again */
		for(size_t j=0; j < neurons_.size(); j++) {
			if(j!=0) {
				neurons_[j].setBuffer(true);
			}
		}	
	}			
}
